  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <Container maxWidth="lg" className="relative z-10">
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="text-center text-white py-20">
          <motion.div variants={fadeInUp}>
            <Typography variant="h1" className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold mb-6 leading-tight">
              {t("hero.title")}
            </Typography>
          </motion.div>
          <motion.div variants={fadeInUp}>
            <Typography variant="h5" className="text-lg md:text-xl lg:text-2xl mb-10 text-white/90 max-w-2xl mx-auto font-light">
              {t("hero.subtitle")}
            </Typography>
          </motion.div>
          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            <CTAButton variant="outlined" onClick={() => handleScroll("#templates")}>
              <span className="text-white border-white">{t("hero.secondaryCTA")}</span>
            </CTAButton>
          </motion.div>
          <motion.div variants={fadeIn} className="mt-16">
            <div className="max-w-sm mx-auto bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 shadow-2xl">
              <div className="bg-white/20 rounded-xl p-4 mb-4">
                ...
                <Typography variant="h6" className="text-white font-heading">{t("hero.mockup.name")}</Typography>
                <Typography variant="body2" className="text-white/80">{t("hero.mockup.date")}</Typography>
              </div>
              <div className="flex gap-2 text-xs text-white/70">
                <span className="bg-white/20 px-3 py-1 rounded-full">{t("hero.mockup.music")}</span>
                <span className="bg-white/20 px-3 py-1 rounded-full">{t("hero.mockup.map")}</span>
                <span className="bg-white/20 px-3 py-1 rounded-full">{t("hero.mockup.rsvp")}</span>
              </div>
            </div>
          </motion.div>
        </motion.div>
